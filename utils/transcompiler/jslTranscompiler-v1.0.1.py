...
def addLeaf( prefix, treeFile, branchLevel, itemName):
	treeFile.write( "\n" )
	for i in range( 0, branchLevel ):
		treeFile.write( "\t" )
	treeFile.write( prefix + itemName )
				...
				with io.open( subDirectory + fileName, "r", encoding="utf-8" ) as file:
					for line in file:
						...
						if line.startswith( publicModuleKeyword ):
							...
							addLeaf( "* ", treeFile, branchLevel, subModuleName )
							branchLevel += 1
							...
							addLeaf( "+ ", treeFile, branchLevel, className )
							branchLevel += 1
							line = "/*" + line[ : -1 ] + "*/"
							print( line )
							buildFile.write( line )
	...
	for line in file:
		...
		if line.find( publicKeyword ) > -1:
			...
			if line.find( " function " ) > -1:
				...
				print( memberName )
				addLeaf( "uf ", treeFile, branchLevel, memberName )
			...
			# Else if variable/property
				...
				memberName = line[ memberNameStartIndex : memberNameEndIndex ]
				print( memberName )
				addLeaf( "uv ", treeFile, branchLevel, memberName )
				publicMembers.append( memberName )
				line = line.replace( publicKeyword, "/*" + publicKeyword + "*/", 1 )
				print( line )
		...
		# If line is private member opener
			...
			if line.find( " function " ) > -1:
				memberNameStartIndex = len( privateKeyword ) + len( " function " )
				memberNameEndIndex  = line.find( "(", memberNameStartIndex )
				memberName = line[ memberNameStartIndex : memberNameEndIndex ]
				print( memberName )
				addLeaf( "rf ", treeFile, branchLevel, memberName )
				line = line.replace( privateKeyword, "/*" + privateKeyword + "*/", 1 )
				print( line )

			# Else if private variable/property
			elif line.find( " var " ) > -1:
				memberNameStartIndex = len( privateKeyword ) + len( " var " )
				memberNameEndIndex  = line.find( " = ", memberNameStartIndex )
				memberName = line[ memberNameStartIndex : memberNameEndIndex ]
				print( memberName )
				addLeaf( "rv ", treeFile, branchLevel, memberName )
				line = line.replace( privateKeyword, "/*" + privateKeyword + "*/", 1 )
				print( line )
...
def compileClass( file, className ):
	...
	for line in file:
		...
		# If line is static property
			...
			memberNameStartIndex = len( staticPropertyKeyword ) + 1
			memberNameEndIndex  = line.find( " = ", memberNameStartIndex )
			memberName = line[ memberNameStartIndex : memberNameEndIndex ]
			print( memberName )
			addLeaf( "sp ", treeFile, branchLevel, memberName )
		...
		# If line is instance property
			...
			memberNameStartIndex = len( instancePropertyKeyword ) + 1
			memberNameEndIndex  = line.find( " = ", memberNameStartIndex )
			memberName = line[ memberNameStartIndex : memberNameEndIndex ]
			print( memberName )
			addLeaf( "ip ", treeFile, branchLevel, memberName )
			line = line.replace( instancePropertyKeyword + " ", "\tthis." )

		# If line is static method
		elif line.startswith( staticMethodKeyword ):
			memberNameStartIndex = len( staticMethodKeyword ) + 1
			memberNameEndIndex  = line.find( " = ", memberNameStartIndex )
			memberName = line[ memberNameStartIndex : memberNameEndIndex ]
			print( memberName )
			addLeaf( "sm ", treeFile, branchLevel, memberName )
			line = line.replace( staticMethodKeyword + " ", className + "." )

		# If line is instance method
		elif line.startswith( instanceMethodKeyword ):
			memberNameStartIndex = len( instanceMethodKeyword ) + 1
			memberNameEndIndex  = line.find( " = ", memberNameStartIndex )
			memberName = line[ memberNameStartIndex : memberNameEndIndex ]
			print( memberName )
			addLeaf( "im ", treeFile, branchLevel, memberName )
